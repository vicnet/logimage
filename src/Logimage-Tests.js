define("amber-logimage/Logimage-Tests", ["amber/boot", "amber_core/SUnit"], function($boot){"use strict";
var $core=$boot.api,nil=$boot.nil,$recv=$boot.asReceiver,$globals=$boot.globals;
$core.addPackage('Logimage-Tests');
$core.packages["Logimage-Tests"].innerEval = function (expr) { return eval(expr); };
$core.packages["Logimage-Tests"].transport = {"type":"amd","amdNamespace":"amber-logimage"};

$core.addClass('LogimageTest', $globals.TestCase, ['logimage'], 'Logimage-Tests');
$core.addMethod(
$core.method({
selector: "setUp",
protocol: 'running',
fn: function (){
var self=this;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
self["@logimage"]=$recv($globals.Logimage)._simple();
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"setUp",{},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "setUp\x0a\x09logimage := Logimage simple",
referencedClasses: ["Logimage"],
//>>excludeEnd("ide");
messageSends: ["simple"]
}),
$globals.LogimageTest);

$core.addMethod(
$core.method({
selector: "testCellsAccesses",
protocol: 'tests',
fn: function (){
var self=this;
var c;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
var $4,$5,$3,$2,$7,$8,$6,$1,$12,$13,$11,$10,$15,$16,$14,$9,$20,$21,$19,$18,$23,$22,$17,$27,$28,$26,$25,$30,$31,$29,$24,$35,$36,$34,$33,$38,$39,$37,$32,$43,$44,$42,$41,$46,$47,$45,$40,$51,$52,$50,$49,$54,$55,$53,$48,$59,$60,$58,$57,$62,$63,$61,$56,$64,$65,$67,$69,$70,$68,$66,$72,$74,$75,$73,$71,$77,$79,$80,$78,$76,$83,$82,$85,$86,$84,$81,$89,$88,$91,$92,$90,$87,$94,$96,$97,$95,$93,$101,$102,$100,$99,$104,$105,$103,$98,$109,$110,$108,$107,$112,$113,$111,$106,$117,$118,$116,$115,$120,$119,$114,$124,$125,$123,$122,$127,$126,$121;
$4=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=1;
//>>excludeEnd("ctx");
$recv($4)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=1;
//>>excludeEnd("ctx");
$recv($4)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=1;
//>>excludeEnd("ctx");
$recv($4)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=2;
//>>excludeEnd("ctx");
$recv($4)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=2;
//>>excludeEnd("ctx");
$5=$recv($4)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=1;
//>>excludeEnd("ctx");
$3=$5;
$2=$recv($3)._firstDones();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstDones"]=1;
//>>excludeEnd("ctx");
$7=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=2;
//>>excludeEnd("ctx");
$recv($7)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=3;
//>>excludeEnd("ctx");
$recv($7)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=3;
//>>excludeEnd("ctx");
$recv($7)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=4;
//>>excludeEnd("ctx");
$8=$recv($7)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=4;
//>>excludeEnd("ctx");
$6=$8;
$1=$recv($2).__eq($6);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=1;
//>>excludeEnd("ctx");
self._assert_($1);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=1;
//>>excludeEnd("ctx");
$12=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=3;
//>>excludeEnd("ctx");
$recv($12)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=5;
//>>excludeEnd("ctx");
$recv($12)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=5;
//>>excludeEnd("ctx");
$recv($12)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=6;
//>>excludeEnd("ctx");
$recv($12)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=6;
//>>excludeEnd("ctx");
$13=$recv($12)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=2;
//>>excludeEnd("ctx");
$11=$13;
$10=$recv($11)._firstDones();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstDones"]=2;
//>>excludeEnd("ctx");
$15=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=4;
//>>excludeEnd("ctx");
$recv($15)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=7;
//>>excludeEnd("ctx");
$recv($15)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=7;
//>>excludeEnd("ctx");
$16=$recv($15)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=8;
//>>excludeEnd("ctx");
$14=$16;
$9=$recv($10).__eq($14);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=2;
//>>excludeEnd("ctx");
self._assert_($9);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=2;
//>>excludeEnd("ctx");
$20=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=5;
//>>excludeEnd("ctx");
$recv($20)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=9;
//>>excludeEnd("ctx");
$recv($20)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=8;
//>>excludeEnd("ctx");
$21=$recv($20)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=3;
//>>excludeEnd("ctx");
$19=$21;
$18=$recv($19)._firstDones();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstDones"]=3;
//>>excludeEnd("ctx");
$23=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=6;
//>>excludeEnd("ctx");
$22=$recv($23)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=10;
//>>excludeEnd("ctx");
$17=$recv($18).__eq($22);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=3;
//>>excludeEnd("ctx");
self._assert_($17);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=3;
//>>excludeEnd("ctx");
$27=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=7;
//>>excludeEnd("ctx");
$recv($27)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=11;
//>>excludeEnd("ctx");
$recv($27)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=9;
//>>excludeEnd("ctx");
$28=$recv($27)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=10;
//>>excludeEnd("ctx");
$26=$28;
$25=$recv($26)._firstDones();
$30=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=8;
//>>excludeEnd("ctx");
$recv($30)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=12;
//>>excludeEnd("ctx");
$recv($30)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=11;
//>>excludeEnd("ctx");
$31=$recv($30)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=12;
//>>excludeEnd("ctx");
$29=$31;
$24=$recv($25).__eq($29);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=4;
//>>excludeEnd("ctx");
self._assert_($24);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=4;
//>>excludeEnd("ctx");
$35=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=9;
//>>excludeEnd("ctx");
$recv($35)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=13;
//>>excludeEnd("ctx");
$recv($35)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=13;
//>>excludeEnd("ctx");
$recv($35)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=4;
//>>excludeEnd("ctx");
$recv($35)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=14;
//>>excludeEnd("ctx");
$36=$recv($35)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=14;
//>>excludeEnd("ctx");
$34=$36;
$33=$recv($34)._lastDones();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["lastDones"]=1;
//>>excludeEnd("ctx");
$38=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=10;
//>>excludeEnd("ctx");
$recv($38)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=15;
//>>excludeEnd("ctx");
$39=$recv($38)._pos_((5));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=1;
//>>excludeEnd("ctx");
$37=$39;
$32=$recv($33).__eq($37);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=5;
//>>excludeEnd("ctx");
self._assert_($32);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=5;
//>>excludeEnd("ctx");
$43=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=11;
//>>excludeEnd("ctx");
$recv($43)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=16;
//>>excludeEnd("ctx");
$recv($43)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=5;
//>>excludeEnd("ctx");
$recv($43)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=17;
//>>excludeEnd("ctx");
$recv($43)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=15;
//>>excludeEnd("ctx");
$recv($43)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=16;
//>>excludeEnd("ctx");
$44=$recv($43)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=18;
//>>excludeEnd("ctx");
$42=$44;
$41=$recv($42)._lastDones();
$46=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=12;
//>>excludeEnd("ctx");
$recv($46)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=19;
//>>excludeEnd("ctx");
$recv($46)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=17;
//>>excludeEnd("ctx");
$recv($46)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=18;
//>>excludeEnd("ctx");
$recv($46)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=20;
//>>excludeEnd("ctx");
$47=$recv($46)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=2;
//>>excludeEnd("ctx");
$45=$47;
$40=$recv($41).__eq($45);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=6;
//>>excludeEnd("ctx");
self._assert_($40);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=6;
//>>excludeEnd("ctx");
$51=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=13;
//>>excludeEnd("ctx");
$recv($51)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=21;
//>>excludeEnd("ctx");
$recv($51)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=19;
//>>excludeEnd("ctx");
$52=$recv($51)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=20;
//>>excludeEnd("ctx");
$50=$52;
$49=$recv($50)._copyFrom_((1));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["copyFrom:"]=1;
//>>excludeEnd("ctx");
$54=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=14;
//>>excludeEnd("ctx");
$recv($54)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=22;
//>>excludeEnd("ctx");
$recv($54)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=21;
//>>excludeEnd("ctx");
$55=$recv($54)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=22;
//>>excludeEnd("ctx");
$53=$55;
$48=$recv($49).__eq($53);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=7;
//>>excludeEnd("ctx");
self._assert_($48);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=7;
//>>excludeEnd("ctx");
$59=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=15;
//>>excludeEnd("ctx");
$recv($59)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=23;
//>>excludeEnd("ctx");
$recv($59)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=23;
//>>excludeEnd("ctx");
$60=$recv($59)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=24;
//>>excludeEnd("ctx");
$58=$60;
$57=$recv($58)._copyFrom_((2));
$62=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=16;
//>>excludeEnd("ctx");
$recv($62)._pos_((2));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=3;
//>>excludeEnd("ctx");
$recv($62)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=25;
//>>excludeEnd("ctx");
$63=$recv($62)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=26;
//>>excludeEnd("ctx");
$61=$63;
$56=$recv($57).__eq($61);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=8;
//>>excludeEnd("ctx");
self._assert_($56);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=8;
//>>excludeEnd("ctx");
$64=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=17;
//>>excludeEnd("ctx");
$recv($64)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=24;
//>>excludeEnd("ctx");
$recv($64)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=25;
//>>excludeEnd("ctx");
$recv($64)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=27;
//>>excludeEnd("ctx");
$recv($64)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=26;
//>>excludeEnd("ctx");
$recv($64)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=6;
//>>excludeEnd("ctx");
$recv($64)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=28;
//>>excludeEnd("ctx");
$65=$recv($64)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=27;
//>>excludeEnd("ctx");
c=$65;
$67=$recv(c)._trimFirst();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trimFirst"]=1;
//>>excludeEnd("ctx");
$69=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=18;
//>>excludeEnd("ctx");
$recv($69)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=29;
//>>excludeEnd("ctx");
$recv($69)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=28;
//>>excludeEnd("ctx");
$recv($69)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=7;
//>>excludeEnd("ctx");
$recv($69)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=30;
//>>excludeEnd("ctx");
$recv($69)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=29;
//>>excludeEnd("ctx");
$70=$recv($69)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=4;
//>>excludeEnd("ctx");
$68=$70;
$66=$recv($67).__eq($68);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=9;
//>>excludeEnd("ctx");
self._assert_($66);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=9;
//>>excludeEnd("ctx");
$72=$recv(c)._trimLast();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trimLast"]=1;
//>>excludeEnd("ctx");
$74=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=19;
//>>excludeEnd("ctx");
$recv($74)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=30;
//>>excludeEnd("ctx");
$recv($74)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=31;
//>>excludeEnd("ctx");
$recv($74)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=31;
//>>excludeEnd("ctx");
$recv($74)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=32;
//>>excludeEnd("ctx");
$recv($74)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=8;
//>>excludeEnd("ctx");
$75=$recv($74)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=32;
//>>excludeEnd("ctx");
$73=$75;
$71=$recv($72).__eq($73);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=10;
//>>excludeEnd("ctx");
self._assert_($71);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=10;
//>>excludeEnd("ctx");
$77=$recv(c)._trim();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trim"]=1;
//>>excludeEnd("ctx");
$79=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=20;
//>>excludeEnd("ctx");
$recv($79)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=33;
//>>excludeEnd("ctx");
$recv($79)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=33;
//>>excludeEnd("ctx");
$recv($79)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=9;
//>>excludeEnd("ctx");
$recv($79)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=34;
//>>excludeEnd("ctx");
$80=$recv($79)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=5;
//>>excludeEnd("ctx");
$78=$80;
$76=$recv($77).__eq($78);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=11;
//>>excludeEnd("ctx");
self._assert_($76);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=11;
//>>excludeEnd("ctx");
$83=$recv(c)._trim();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trim"]=2;
//>>excludeEnd("ctx");
$82=$recv($83)._trimFirst();
$85=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=21;
//>>excludeEnd("ctx");
$recv($85)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=35;
//>>excludeEnd("ctx");
$recv($85)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=34;
//>>excludeEnd("ctx");
$recv($85)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=10;
//>>excludeEnd("ctx");
$recv($85)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=36;
//>>excludeEnd("ctx");
$86=$recv($85)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=6;
//>>excludeEnd("ctx");
$84=$86;
$81=$recv($82).__eq($84);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=12;
//>>excludeEnd("ctx");
self._assert_($81);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=12;
//>>excludeEnd("ctx");
$89=$recv(c)._trim();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trim"]=3;
//>>excludeEnd("ctx");
$88=$recv($89)._trimLast();
$91=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=22;
//>>excludeEnd("ctx");
$recv($91)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=37;
//>>excludeEnd("ctx");
$recv($91)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=35;
//>>excludeEnd("ctx");
$recv($91)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=11;
//>>excludeEnd("ctx");
$recv($91)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=38;
//>>excludeEnd("ctx");
$92=$recv($91)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=7;
//>>excludeEnd("ctx");
$90=$92;
$87=$recv($88).__eq($90);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=13;
//>>excludeEnd("ctx");
self._assert_($87);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=13;
//>>excludeEnd("ctx");
$94=$recv($recv(c)._trim())._trim();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["trim"]=4;
//>>excludeEnd("ctx");
$96=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=23;
//>>excludeEnd("ctx");
$recv($96)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=39;
//>>excludeEnd("ctx");
$recv($96)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=36;
//>>excludeEnd("ctx");
$recv($96)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=12;
//>>excludeEnd("ctx");
$recv($96)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=40;
//>>excludeEnd("ctx");
$97=$recv($96)._pos_((3));
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["pos:"]=8;
//>>excludeEnd("ctx");
$95=$97;
$93=$recv($94).__eq($95);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=14;
//>>excludeEnd("ctx");
self._assert_($93);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=14;
//>>excludeEnd("ctx");
$101=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=24;
//>>excludeEnd("ctx");
$recv($101)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=13;
//>>excludeEnd("ctx");
$recv($101)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=41;
//>>excludeEnd("ctx");
$recv($101)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=42;
//>>excludeEnd("ctx");
$102=$recv($101)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=14;
//>>excludeEnd("ctx");
$100=$102;
$99=$recv($100)._firstNotSpace();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstNotSpace"]=1;
//>>excludeEnd("ctx");
$104=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=25;
//>>excludeEnd("ctx");
$recv($104)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=15;
//>>excludeEnd("ctx");
$recv($104)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=43;
//>>excludeEnd("ctx");
$recv($104)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=44;
//>>excludeEnd("ctx");
$105=$recv($104)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=16;
//>>excludeEnd("ctx");
$103=$105;
$98=$recv($99).__eq($103);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=15;
//>>excludeEnd("ctx");
self._assert_($98);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=15;
//>>excludeEnd("ctx");
$109=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=26;
//>>excludeEnd("ctx");
$recv($109)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=17;
//>>excludeEnd("ctx");
$recv($109)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=45;
//>>excludeEnd("ctx");
$recv($109)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=46;
//>>excludeEnd("ctx");
$110=$recv($109)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=37;
//>>excludeEnd("ctx");
$108=$110;
$107=$recv($108)._firstNotSpace();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstNotSpace"]=2;
//>>excludeEnd("ctx");
$112=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=27;
//>>excludeEnd("ctx");
$recv($112)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=18;
//>>excludeEnd("ctx");
$recv($112)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=47;
//>>excludeEnd("ctx");
$113=$recv($112)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=48;
//>>excludeEnd("ctx");
$111=$113;
$106=$recv($107).__eq($111);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=16;
//>>excludeEnd("ctx");
self._assert_($106);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=16;
//>>excludeEnd("ctx");
$117=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=28;
//>>excludeEnd("ctx");
$recv($117)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=19;
//>>excludeEnd("ctx");
$recv($117)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=38;
//>>excludeEnd("ctx");
$recv($117)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=49;
//>>excludeEnd("ctx");
$118=$recv($117)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=20;
//>>excludeEnd("ctx");
$116=$118;
$115=$recv($116)._firstNotSpace();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["firstNotSpace"]=3;
//>>excludeEnd("ctx");
$120=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=29;
//>>excludeEnd("ctx");
$119=$recv($120)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=21;
//>>excludeEnd("ctx");
$114=$recv($115).__eq($119);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=17;
//>>excludeEnd("ctx");
self._assert_($114);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=17;
//>>excludeEnd("ctx");
$124=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=30;
//>>excludeEnd("ctx");
$recv($124)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=39;
//>>excludeEnd("ctx");
$recv($124)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=40;
//>>excludeEnd("ctx");
$recv($124)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=22;
//>>excludeEnd("ctx");
$recv($124)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=50;
//>>excludeEnd("ctx");
$recv($124)._s();
$125=$recv($124)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=23;
//>>excludeEnd("ctx");
$123=$125;
$122=$recv($123)._firstNotSpace();
$127=$recv($globals.Cells)._new();
$recv($127)._u();
$recv($127)._b();
$126=$recv($127)._pos_((3));
$121=$recv($122).__eq($126);
self._assert_($121);
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"testCellsAccesses",{c:c},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "testCellsAccesses\x0a\x09| c |\x0a\x09\x22test firstDones\x22\x0a\x09self assert: (Cells new s; b; b; s; u) firstDones = (Cells new s; b; b; s).\x0a\x09self assert: (Cells new s; b; s; b; u) firstDones = (Cells new s; b; s).\x0a\x09self assert: (Cells new s; b; u) firstDones = (Cells new s).\x0a\x09self assert: (Cells new s; b; b) firstDones = (Cells new s; b; b).\x0a\x09\x22test last dones\x22\x0a\x09self assert: (Cells new s; b; u; b; s) lastDones = (Cells new s; pos: 5).\x0a\x09self assert: (Cells new s; u; s; b; b; s) lastDones = (Cells new s; b; b; s; pos: 3).\x0a\x09\x22test copyFrom: with number\x22\x0a\x09self assert: ((Cells new s; b; b) copyFrom: 1) = (Cells new s; b; b).\x0a\x09self assert: ((Cells new s; b; b) copyFrom: 2) = (Cells new pos: 2; b; b).\x0a\x09\x22test trim functions\x22\x0a\x09c := Cells new s; s; b; s; u; b; s.\x0a\x09self assert: c trimFirst = (Cells new b; s; u; b; s; pos: 3).\x0a\x09self assert: c trimLast = (Cells new s; s; b; s; u; b).\x0a\x09self assert: c trim = (Cells new b; s; u; b; pos: 3).\x0a\x09self assert: c trim trimFirst = (Cells new b; s; u; b; pos: 3).\x0a\x09self assert: c trim trimLast = (Cells new b; s; u; b; pos: 3).\x0a\x09self assert: c trim trim = (Cells new b; s; u; b; pos: 3).\x0a\x09\x22test firstNotSpace\x22\x0a\x09self assert: (Cells new u; b; b; u) firstNotSpace = (Cells new u; b; b; u).\x0a\x09self assert: (Cells new u; b; b; s) firstNotSpace = (Cells new u; b; b).\x0a\x09self assert: (Cells new u; s; b; u) firstNotSpace = (Cells new u).\x0a\x09self assert: (Cells new s; s; u; b; s; u) firstNotSpace = (Cells new u; b; pos: 3).\x0a\x09\x22\x0a\x09c := Cells new s; b; b; s; u; b; s.\x0a\x09self assert: c lastNumbers = { }.\x0a\x09self assert: (Cells new s; b; b; s; s; b; s) lastNumbers = { 2. 1 }.\x0a\x09self assert: c firstBox = #{ #pos -> 2. #size -> 2 }.\x0a\x09self assert: c numbers = { 2. 1 }.\x0a\x09self assert: ((Cells new u;s;u;b) firstFreeSpace) = #{ #pos -> 1. #size -> 1 }.\x0a\x09self assert: ((Cells new b;s;u;u) firstFreeSpace) = #{ #pos -> 3. #size -> 2 }.\x0a\x09self assert: ((Cells new s;u;s;u;b) firstNotSpace) = #{ #pos -> 2. #size -> 1 }.\x0a\x09self assert: ((Cells new s;b;u;s;u;u) firstNotSpace) = #{ #pos -> 2. #size -> 2 }.\x22",
referencedClasses: ["Cells"],
//>>excludeEnd("ide");
messageSends: ["assert:", "=", "firstDones", "s", "new", "b", "u", "lastDones", "pos:", "copyFrom:", "trimFirst", "trimLast", "trim", "firstNotSpace"]
}),
$globals.LogimageTest);

$core.addMethod(
$core.method({
selector: "testLineAccesses",
protocol: 'tests',
fn: function (){
var self=this;
var l;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
var $1,$3,$4,$2,$6,$8,$9,$11,$12,$10,$7,$5,$16,$18,$19,$17,$15,$14,$21,$23,$24,$22,$20,$13,$28,$30,$31,$29,$27,$26,$33,$35,$34,$32,$25;
$1=$recv($globals.Hint)._new_([(2),(1)]);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:"]=1;
//>>excludeEnd("ctx");
$3=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=1;
//>>excludeEnd("ctx");
$recv($3)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=1;
//>>excludeEnd("ctx");
$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=1;
//>>excludeEnd("ctx");
$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=2;
//>>excludeEnd("ctx");
$recv($3)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=2;
//>>excludeEnd("ctx");
$recv($3)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=1;
//>>excludeEnd("ctx");
$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=3;
//>>excludeEnd("ctx");
$4=$recv($3)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=3;
//>>excludeEnd("ctx");
$2=$4;
l=$recv($globals.Line)._hint_cells_($1,$2);
$6=$recv(l)._reversed();
$8=$recv($globals.Line)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=2;
//>>excludeEnd("ctx");
$9=$recv($globals.Hint)._new_([(1),(2)]);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:"]=2;
//>>excludeEnd("ctx");
$recv($8)._hint_($9);
$11=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=3;
//>>excludeEnd("ctx");
$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=4;
//>>excludeEnd("ctx");
$recv($11)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=4;
//>>excludeEnd("ctx");
$recv($11)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=2;
//>>excludeEnd("ctx");
$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=5;
//>>excludeEnd("ctx");
$recv($11)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=5;
//>>excludeEnd("ctx");
$recv($11)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=6;
//>>excludeEnd("ctx");
$12=$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=6;
//>>excludeEnd("ctx");
$10=$12;
$7=$recv($8)._cells_($10);
$5=$recv($6).__eq($7);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=1;
//>>excludeEnd("ctx");
self._assert_($5);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=1;
//>>excludeEnd("ctx");
$16=$recv($globals.Hint)._new_([(2),(1)]);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:"]=3;
//>>excludeEnd("ctx");
$18=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=4;
//>>excludeEnd("ctx");
$recv($18)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=7;
//>>excludeEnd("ctx");
$recv($18)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=7;
//>>excludeEnd("ctx");
$recv($18)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=8;
//>>excludeEnd("ctx");
$recv($18)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=3;
//>>excludeEnd("ctx");
$recv($18)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=8;
//>>excludeEnd("ctx");
$recv($18)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=9;
//>>excludeEnd("ctx");
$19=$recv($18)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=9;
//>>excludeEnd("ctx");
$17=$19;
$15=$recv($globals.Line)._new_cells_($16,$17);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:cells:"]=1;
//>>excludeEnd("ctx");
$14=$recv($15)._reducedLast();
$21=$recv($globals.Hint)._new_([(2)]);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:"]=4;
//>>excludeEnd("ctx");
$23=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=5;
//>>excludeEnd("ctx");
$recv($23)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=10;
//>>excludeEnd("ctx");
$recv($23)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=10;
//>>excludeEnd("ctx");
$recv($23)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=11;
//>>excludeEnd("ctx");
$24=$recv($23)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=4;
//>>excludeEnd("ctx");
$22=$24;
$20=$recv($globals.Line)._new_cells_($21,$22);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:cells:"]=2;
//>>excludeEnd("ctx");
$13=$recv($14).__eq($20);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=2;
//>>excludeEnd("ctx");
self._assert_($13);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=2;
//>>excludeEnd("ctx");
$28=$recv($globals.Hint)._new_([(2),(1)]);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:"]=5;
//>>excludeEnd("ctx");
$30=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=6;
//>>excludeEnd("ctx");
$recv($30)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=11;
//>>excludeEnd("ctx");
$recv($30)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=12;
//>>excludeEnd("ctx");
$recv($30)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=13;
//>>excludeEnd("ctx");
$recv($30)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=12;
//>>excludeEnd("ctx");
$recv($30)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=5;
//>>excludeEnd("ctx");
$recv($30)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=14;
//>>excludeEnd("ctx");
$31=$recv($30)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=13;
//>>excludeEnd("ctx");
$29=$31;
$27=$recv($globals.Line)._new_cells_($28,$29);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new:cells:"]=3;
//>>excludeEnd("ctx");
$26=$recv($27)._reducedFirst();
$33=$recv($globals.Hint)._new_([(1)]);
$35=$recv($globals.Cells)._new();
$recv($35)._u();
$recv($35)._b();
$recv($35)._s();
$34=$recv($35)._pos_((5));
$32=$recv($globals.Line)._new_cells_($33,$34);
$25=$recv($26).__eq($32);
self._assert_($25);
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"testLineAccesses",{l:l},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "testLineAccesses\x0a\x09| l |\x0a\x09l := Line\x0a\x09\x09hint:  ( Hint new: { 2 . 1 } )\x0a\x09\x09cells: ( Cells new s; b; b; s; u; b; s).\x0a\x09self assert: l reversed =\x0a\x09\x09(Line new hint: (Hint new: { 1. 2}); cells: (Cells new s; b; u; s; b; b; s)).\x0a\x09self assert: (Line new: (Hint new: {2. 1}) cells: (Cells new s; b; b; u; s; b; s)) reducedLast =\x0a\x09\x09(Line new: (Hint new: {2}) cells: (Cells new s; b; b; u)).\x0a\x09self assert: (Line new: (Hint new: {2. 1}) cells: (Cells new s; b; b; s; u; b; s)) reducedFirst =\x0a\x09\x09(Line new: (Hint new: {1}) cells: (Cells new u; b; s; pos: 5)).",
referencedClasses: ["Line", "Hint", "Cells"],
//>>excludeEnd("ide");
messageSends: ["hint:cells:", "new:", "s", "new", "b", "u", "assert:", "=", "reversed", "hint:", "cells:", "reducedLast", "new:cells:", "reducedFirst", "pos:"]
}),
$globals.LogimageTest);

$core.addMethod(
$core.method({
selector: "testLogimageAccesses",
protocol: 'tests',
fn: function (){
var self=this;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
self._assert_($recv($recv(self["@logimage"])._colCellsAt_((1)))._notNil());
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"testLogimageAccesses",{},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "testLogimageAccesses\x0a\x09self assert: (logimage colCellsAt: 1) notNil",
referencedClasses: [],
//>>excludeEnd("ide");
messageSends: ["assert:", "notNil", "colCellsAt:"]
}),
$globals.LogimageTest);

$core.addMethod(
$core.method({
selector: "testStratNotReached",
protocol: 'tests',
fn: function (){
var self=this;
var l,sol;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
var $1,$3,$2,$5,$6,$4,$7,$9,$11,$10,$8;
$1=$recv($globals.Line)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=1;
//>>excludeEnd("ctx");
$3=$recv($globals.Hint)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=2;
//>>excludeEnd("ctx");
$2=$recv($3)._numbers_([(2)]);
$recv($1)._hint_($2);
$5=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=3;
//>>excludeEnd("ctx");
$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=1;
//>>excludeEnd("ctx");
$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=2;
//>>excludeEnd("ctx");
$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=3;
//>>excludeEnd("ctx");
$recv($5)._b();
$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=4;
//>>excludeEnd("ctx");
$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=5;
//>>excludeEnd("ctx");
$6=$recv($5)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=6;
//>>excludeEnd("ctx");
$4=$6;
l=$recv($1)._cells_($4);
$7=$recv($globals.StratNotReached)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=4;
//>>excludeEnd("ctx");
sol=$recv($7)._analyse_(l);
$9=sol;
$11=$recv($globals.Cells)._new();
$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=1;
//>>excludeEnd("ctx");
$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=2;
//>>excludeEnd("ctx");
$recv($11)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=7;
//>>excludeEnd("ctx");
$recv($11)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=8;
//>>excludeEnd("ctx");
$recv($11)._u();
$recv($11)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=3;
//>>excludeEnd("ctx");
$10=$recv($11)._s();
$8=$recv($9).__eq($10);
self._assert_($8);
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"testStratNotReached",{l:l,sol:sol},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "testStratNotReached\x0a\x09| l sol |\x0a\x09l := Line new\x0a\x09\x09hint: (Hint new numbers: { 2 });\x0a\x09\x09cells: (Cells new u; u; u; b; u; u; u).\x0a\x09sol := StratNotReached new analyse: l.\x0a\x09self assert: sol = (Cells new s; s; u; u; u; s; s)",
referencedClasses: ["Line", "Hint", "Cells", "StratNotReached"],
//>>excludeEnd("ide");
messageSends: ["hint:", "new", "numbers:", "cells:", "u", "b", "analyse:", "assert:", "=", "s"]
}),
$globals.LogimageTest);

$core.addMethod(
$core.method({
selector: "testStratXDone",
protocol: 'tests',
fn: function (){
var self=this;
var l,sol,res;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
return $core.withContext(function($ctx1) {
//>>excludeEnd("ctx");
var $1,$3,$4,$2,$5,$7,$9,$10,$8,$6,$11,$13,$15,$14,$12;
$1=$recv($globals.Line)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=1;
//>>excludeEnd("ctx");
$recv($1)._hint_($recv($globals.Hint)._new_([(1),(2)]));
$3=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=2;
//>>excludeEnd("ctx");
$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=1;
//>>excludeEnd("ctx");
$recv($3)._u();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["u"]=1;
//>>excludeEnd("ctx");
$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=2;
//>>excludeEnd("ctx");
$4=$recv($3)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=3;
//>>excludeEnd("ctx");
$2=$4;
l=$recv($1)._cells_($2);
$5=$recv($globals.StratFirstDone)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=3;
//>>excludeEnd("ctx");
sol=$recv($5)._analyse_(l);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["analyse:"]=1;
//>>excludeEnd("ctx");
$7=sol;
$9=$recv($globals.Cells)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=4;
//>>excludeEnd("ctx");
$recv($9)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=4;
//>>excludeEnd("ctx");
$10=$recv($9)._s();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["s"]=1;
//>>excludeEnd("ctx");
$8=$10;
$6=$recv($7).__eq($8);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["="]=1;
//>>excludeEnd("ctx");
self._assert_($6);
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["assert:"]=1;
//>>excludeEnd("ctx");
$11=$recv($globals.StratLastDone)._new();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["new"]=5;
//>>excludeEnd("ctx");
sol=$recv($11)._analyse_(l);
$13=sol;
$15=$recv($globals.Cells)._new();
$recv($15)._u();
$recv($15)._s();
$recv($15)._b();
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
$ctx1.sendIdx["b"]=5;
//>>excludeEnd("ctx");
$14=$recv($15)._b();
$12=$recv($13).__eq($14);
self._assert_($12);
return self;
//>>excludeStart("ctx", pragmas.excludeDebugContexts);
}, function($ctx1) {$ctx1.fill(self,"testStratXDone",{l:l,sol:sol,res:res},$globals.LogimageTest)});
//>>excludeEnd("ctx");
},
//>>excludeStart("ide", pragmas.excludeIdeData);
args: [],
source: "testStratXDone\x0a\x09| l sol res |\x0a\x09l := Line new\x0a\x09\x09hint: (Hint new: {1. 2});\x0a\x09\x09cells: (Cells new b;u;b;b).\x0a\x09sol := StratFirstDone new analyse: l.\x0a\x09self assert: sol = (Cells new b;s).\x0a\x09sol := StratLastDone new analyse: l.\x0a\x09self assert: sol = (Cells new u;s;b;b)",
referencedClasses: ["Line", "Hint", "Cells", "StratFirstDone", "StratLastDone"],
//>>excludeEnd("ide");
messageSends: ["hint:", "new", "new:", "cells:", "b", "u", "analyse:", "assert:", "=", "s"]
}),
$globals.LogimageTest);


});
